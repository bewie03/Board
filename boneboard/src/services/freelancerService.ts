...
class FreelancerServiceClass {
  ...
  // Enhanced method to save service packages with database sync
  async saveServicePackages(walletAddress: string, packages: any[]): Promise<boolean> {
    try {
      ...
      // 2. Update individual profile storage
      const individualKey = `freelancer_${walletAddress}`;
      const individualData = localStorage.getItem(individualKey);
      
      if (individualData) {
        const parsedIndividual = JSON.parse(individualData);
        parsedIndividual.packages = packages;
        localStorage.setItem(individualKey, JSON.stringify(parsedIndividual));
        console.log('✅ Updated individual profile storage');
      }
      
      // 3. Update main profiles list for mini profile sync
      const stored = localStorage.getItem('freelancerProfiles');
      if (stored) {
        const profiles = JSON.parse(stored);
        const profileIndex = profiles.findIndex((p: any) => p.walletAddress === walletAddress);
        if (profileIndex !== -1) {
          profiles[profileIndex].packages = packages;
          localStorage.setItem('freelancerProfiles', JSON.stringify(profiles));
          console.log('✅ Updated main profiles list');
        }
      }
      
      // 4. Update in-memory array with proper service conversion
      const index = this.freelancers.findIndex(f => f.walletAddress === walletAddress);
      if (index !== -1) {
        this.freelancers[index].services = packages.map((pkg: any, pkgIndex: number) => ({
          id: `${this.freelancers[index].id}-service-${pkgIndex}`,
          freelancerId: this.freelancers[index].id,
          walletAddress: walletAddress,
          title: pkg.name,
          description: pkg.description,
          shortDescription: pkg.description.length > 100 ? pkg.description.substring(0, 100) + '...' : pkg.description,
          category: pkg.category || this.freelancers[index].category,
          skills: this.freelancers[index].skills,
          images: [],
          pricing: {
            basic: {
              price: pkg.price || 100,
          ...
          rating: this.freelancers[index].rating,
          reviewCount: this.freelancers[index].reviewCount,
          completedOrders: this.freelancers[index].completedOrders,
          responseTime: this.freelancers[index].responseTime,
          isActive: true,
          createdAt: new Date().toISOString()
        }));
        console.log('✅ Updated in-memory services array');
      }
      
      // 5. Dispatch comprehensive update events
      window.dispatchEvent(new CustomEvent('freelancerProfileUpdated', {
        detail: { walletAddress, packages }
      }));
      
      window.dispatchEvent(new CustomEvent('servicePackagesUpdated', {
        detail: { walletAddress, packages }
      }));
